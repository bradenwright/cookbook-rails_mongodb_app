...
describe command("ls /etc/init.d") do
  its(:stdout) { should match /mongod/ }
end

describe command("mongo --eval 'printjson(rs.status())'") do
  its(:stdout) { should match /\"name\" : \"db1-ubuntu-1404.lxc:27017\",\n\t\t\t\"health\" : 1,\n/ }
  its(:stdout) { should match /\"name\" : \"db2-ubuntu-1404.lxc:27017\",\n\t\t\t\"health\" : 1,\n/ }
end
